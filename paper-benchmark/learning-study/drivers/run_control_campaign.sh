#!/usr/bin/env bash
# The CONTROL campaign: the same eight runs with online learning OFF.
#
#   dynamic        inference only, balanced cost
#   dynamic-ratio  inference only, ratio-only cost
#
# Same weights, same inputs, same everything -- only the SGD is gone. Without
# this arm a falling error curve says nothing: the data gets easier or harder
# on its own as the physics develops, and that motion would be credited to
# learning. The gap between the two curves is the part learning is responsible
# for.
#
...
for cfg in dynamic dynamic-ratio; do
